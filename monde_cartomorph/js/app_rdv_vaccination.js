...
function showTip(d){
  ...
  // console.log(d)
    ...
    soit <span style="font-weight:bold">${String(this_d.couv_dose1).replace('.',',')}</span> pour 1 000 habitants de plus de 55 ans</span></li>`
...
queue()
  ...
  .defer(d3.csv, 'data/rvd_vaccins.csv')
  .await(ready)
  ...
  function ready(error, data0, data) {
    ...
    data.forEach(d =>{
      ...
      d.couv_dose1 = +d['rdv disponibles pour 1 000 habitants de plus de 55 ans'];
      d.population = +d.population;
