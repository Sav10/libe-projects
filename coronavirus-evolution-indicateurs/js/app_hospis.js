...
function showTip(d){
    ...
    let this_indicator = selected_variable == 'rea_pour_100k' ?
     `<p><span style="font-weight:bold;text-transform:uppercase" class='list_element'>Réanimations</span></p>` :
     `<p><span style="font-weight:bold;text-transform:uppercase"" class='list_element'>Hospitalisations</span></p>`

    this_html =  `<ul id='tooltip_content'>${this_indicator}</ul></span>
    <span class='details'><span style="font-weight:bold">${this_d.departement} (${this_d.dep})</span>`
...
var colorScaleHosp = d3.scaleSequential(d3.interpolateOrRd).domain([0,100]);

var colorScaleRea = d3.scaleSequential(d3.interpolateOrRd).domain([0,20]);
...
'tx_incidence': colorIncidence,
 'tx_positivite': colorPositivity,
  'hosp_pour_100k': colorScaleHosp,
   'rea_pour_100k': colorScaleRea
}
...
queue()
  ...
  .defer(d3.csv, 'data/hospi_from_1_october.csv')
  .await(ready)
