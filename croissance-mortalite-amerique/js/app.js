...
const g_x_translation_america = 400;
const g_y_translation_america = 130;

const rangeXEurope = [-400, 500];
const rangeYEurope = [380, 0];

const rangeXAmerica = [-400, 1050];
const rangeYAmerica= [580, 200];
...
if (radius_name != 'this_radius_pop'){

allPaths.filter(d=> d[radius_name.split('_')[1]]==0).attr('visibility', 'hidden')
...
Promise.all([
  ...
  function ready(data) {
    ...
    for (i in allSvgNodes){
      ...
      let mars = this_d ? this_d.mars : null;
      let avril = this_d ? this_d.avril : null;
      let mai = this_d ? this_d.mai : null;
      let juin = this_d ? this_d.juin : null;
      ...
      let radius_mars= Math.round(circleScaleDeaths(mars));
      let radius_avril= Math.round(circleScaleDeaths(avril));
      let radius_mai= Math.round(circleScaleDeaths(mai));
      let radius_juin= Math.round(circleScaleDeaths(juin));
      ...
      d3.select(allSvgNodes[i]).datum({
        ...
        'radius_avril': radius_avril,
        'radius_mai': radius_mai,
        'radius_juin': radius_juin,
...
allPathsAmerica = allPaths.filter(d=>d.continent == 'America' & d.id != 'GL')
...
function responsivefy(svg) {
        ...
        function shareTwitter () {
          var url = encodeURIComponent(window.location.origin + window.location.pathname),
          text = "D'un pays à l'autre, le nombre de morts liés au coronavirus est parfois très varié. Un seul indicateur ne suffisant à l'expliquer, @Libe_Labo en a croisé plusieurs. https://www.liberation.fr/apps/2020/05/covid-19-croiser-les-donnees-pour-mieux-comprendre-la-mortalite/ via @libe",
...
allPaths.style('display', 'initial')
...
function ZoomAmerica(){


allPaths.filter(d=>d.europe != 1).style('display', 'initial')
allPaths.filter(d=>d.continent != 'America').style('display', 'none')
d3.select('path#GL').style('display', 'none')

g.transition().attr('transform', `translate(${g_x_translation_america},${-140}) scale(1.2)`)
...
function force_separate_circles_america(column_y){

  // g.transition().attr('transform', `translate(${-g_x_translation_europe},${g_y_translation_europe}) scale(2)`)


  var features = allPathsAmerica.data();

  simulation = d3.forceSimulation(features)
  .force("y", d3.forceY(function(d) { return column_y ? scaleYEurope(d[column_y]) : 350}).strength(5))
  .force("x", d3.forceX(function(d) { return scaleXAmerica(d.deaths_for_100k) - 700}).strength(8))
  .force("collide", d3.forceCollide(7).radius(d=> d['radius_pop']))
  .stop();
...
const scaleXAmerica = d3.scaleLinear()
.range(rangeXAmerica)
.domain([0, 100]);
...
function drawAxisBottom(customScale){

d3.select('g#axisBottom')
.remove()

if (customScale){
var axisT = d3.axisBottom(customScale);

g.append("g")
.attr('class', 'axis')
.attr('id', 'axisBottom')
    .attr("transform", `translate(${g_x_translation_america},${g_y_translation_america + rangeYAmerica[0]})`)
    .call(axisT);

d3.select('#axisBottom').selectAll('text')
...
12: function(){
transform_all_paths_to_circle('radius_mars')
 mapstate = 2;
 showTipForId('CN')
 tooltip_additional_var = ['Nombre de morts le 1er mars', 'mars']
 allPaths
  .style('fill', d => {
      return color(_.round(100000*d.mars/d.population))
  })
},
13: function(){
transform_all_paths_to_circle('radius_avril')
 mapstate = 2;
 tip.hide()
tooltip_additional_var = ['Nombre de morts le 1er avril', 'avril']
 allPaths
  .style('fill', d => {
      return color(_.round(100000*d.avril/d.population))
  })
},
14: function(){
transform_all_paths_to_circle('radius_mai')
 mapstate = 2;
 tooltip_additional_var = ['Nombre de morts le 1er mai', 'mai']
  allPaths
  .style('fill', d => {
      return color(_.round(100000*d.mai/d.population))
  })
},
15: function(){
transform_all_paths_to_circle('radius_juin')
 mapstate = 2;
 tooltip_additional_var = ['Nombre de morts le 1er juin', 'juin']
  allPaths
  .style('fill', d => {
      return color(_.round(100000*d.juin/d.population))
  })
},
16: function(){
  ZoomReset()
...
18: function(){
force_separate_circles_america()
drawAxisBottom(scaleXAmerica)
